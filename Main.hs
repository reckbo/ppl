...
makeSetUpData projdir =
    let rplc before after s = intercalate after . splitOn before $ s
        ...
        ps =
            concat $
            zipWith
                (pathsMeasureTracts projdir)
                [1 ..]
                (measureTractsFromCaseid "$case")
        ps' =
            concat $
            zipWith (pathsWmql projdir) [1 ..] (wmqlFromCaseid "$case")
    in unlines $
    ...
    \_ caseids ->
         ...
         do usingConfigFile "config/settings.cfg"
            action $
                if null caseids
                    then do
                        projdir <- liftIO $ getCurrentDirectory
                        writeFile' (Paths.outdir </> "SetUpData.sh") $
                            makeSetUpData projdir
                        liftIO $
                            print $
                            map path $ measureTractsFromCaseid "$[case]"
                    else let measureTractNodes =
                                 concatMap measureTractsFromCaseid caseids
                             wmparcInDwiNodes =
                                 concatMap wmparcInDwiFromCaseid caseids
                             ukfNodes = concatMap ukfFromCaseid caseids
                             wmqlNodes = concatMap wmqlFromCaseid caseids
                         in do when
                                   (not $ null measureTractNodes)
                                   (do needs
                                           (measureTractNodes :: [MeasureTractsCsv])
                                       return ())
                               when
                                   (not $ null wmparcInDwiNodes)
                                   (do needs
                                           (wmparcInDwiNodes :: [WmparcInDwi])
                                       return ())
            rules
